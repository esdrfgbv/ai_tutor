...
const SkeletonHero = () => <div className="h-40 rounded-3xl skeleton animate-pulse" />;
const SkeletonStats = () => (
  <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6">
    {[...Array(6)].map((_, i) => <div key={i} className="h-28 rounded-2xl skeleton animate-pulse" />)}
  </div>
);
...
// Memoized Chart Components to prevent re-renders when other APIs resolve
const TrendChart = memo(({ trend }) => (
...
const DailyTimeChart = memo(({ data }) => (
...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    api.get("/learning/profile")
      .then(r => setUserGrade(r.data.grade))
      .finally(() => setLoadingProfile(false));

    api.get("/study-plan")
      .then(r => setStudyPlan(r.data.plan))
  ...
  return (
    <div className="space-y-6 pb-6">
      {/* ══ HERO SECTION ══ */}
      {(loadingStats || loadingProfile) ? <SkeletonHero /> : (
        <motion.div
        ...
        >
          ...
          <div className="relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap gap-2">
              <Link to="/chapters" className="btn-primary text-sm"><BookOpen size={15} /> Study Now</Link>
              <Link to="/quiz" className="btn-ghost text-sm"><ClipboardList size={15} /> Take Test</Link>
              <Link to="/doubts" className="btn-ghost text-sm"><Brain size={15} /> Ask AI</Link>
            </div>
          </div>
        </motion.div>
      )}
      ...
      <div className="grid gap-4 lg:grid-cols-[1.4fr_.6fr]">
        {/* Score Trend */}
        {loadingStats ? <SkeletonChart /> : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl p-5" style={{ background: "rgba(17,17,17,0.9)", border: "1px solid rgba(255,255,255,0.07)", minHeight: 300 }}>
        ...
        {/* Study Plan */}
        {loadingPlan ? <SkeletonPlan /> : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl p-5" style={{ background: "rgba(17,17,17,0.9)", border: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              {(() => {
                ...
                if (todayTasks?.length) {
                  return todayTasks.map((task, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 + idx * 0.08 }} className="flex items-start gap-2.5 p-3 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {loadingStats ? <SkeletonChart /> : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl p-5" style={{ background: "rgba(17,17,17,0.9)", border: "1px solid rgba(255,255,255,0.07)", minHeight: 280 }}>
            <div className="flex items-center gap-2 mb-4">
              <Target size={16} style={{ color: "#adff44" }} />
              <h2 className="font-display font-bold text-white">Subject Performance</h2>
            </div>
            {subjectPerf.length > 0 ? (
              <SubjectChart data={subjectPerf} />
            ) : (
        ...
        {loadingStats ? <SkeletonChart /> : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-2xl p-5" style={{ background: "rgba(17,17,17,0.9)", border: "1px solid rgba(255,255,255,0.07)", minHeight: 280 }}>
            ...
            {stats?.daily_progress?.some((d) => d.minutes > 0) ? (
              <DailyTimeChart data={stats.daily_progress} />
            ) : (
              <div className="flex flex-col items-center justify-center h-40 gap-2">
      ...
      {/* ══ LEADERBOARD ══ */}
      {loadingLeaderboard ? <SkeletonBoard /> : (leaderboard && leaderboard.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 12 }}
          animate={{ opacity: 1, y: 0 }}
